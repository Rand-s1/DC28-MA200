...
# 设置页面配置
st.set_page_config(
    page_title="鹅的DC28×MA200 金叉死叉扫描器",
    page_icon="📈",
    layout="wide",
...
class Config:
    ...
    LIMIT = 250  # 增加K线数量以确保有足够数据计算MA200
    DC_PERIOD = 28
    MA_PERIOD = 200
    SLEEP_BETWEEN_REQUESTS = 0.5
    MAX_WORKERS = 10
    MIN_CANDLES_RELIABLE = 210  # 至少需要210根K线才能可靠计算MA200
...
def create_header():
    """创建页面头部"""
    st.markdown('<h1 class="big-title">📈 鹅的DC28×MA200 金叉死叉扫描器</h1>', unsafe_allow_html=True)
    st.markdown('<p class="subtitle">🚀 DC28通道中轨与MA200均线交叉信号扫描</p>', unsafe_allow_html=True)
...
def create_sidebar():
    ...
    with st.sidebar:
        ...
        signal_types = st.multiselect(
            ...
            options=["金叉信号", "死叉信号"],
            default=["金叉信号", "死叉信号"],
            help="金叉：DC28中轨上穿MA200做多；死叉：DC28中轨下穿MA200做空"
        )
        ...
        return timeframe, signal_types, cross_confirm_bars, show_charts, min_volume, price_position_strict
...
def calculate_dc_ma_signals(df: pd.DataFrame, cross_confirm_bars: int = 2, price_position_strict: bool = True) -> Tuple[Optional[dict], int]:
    """计算DC28与MA200交叉信号"""
    try:
        candle_count = len(df)
        ...
        # 计算MA200
        ma_200 = close_series.rolling(window=Config.MA_PERIOD).mean()
        
        # 检查数据有效性
        if dc_mid.isna().all() or ma_200.isna().all():
            return None, candle_count
        
        # 检测交叉信号
        signal_info = {
            "dc_mid_current": dc_mid.iloc[-1],
            "ma200_current": ma_200.iloc[-1],
        ...
        for i in range(1, min(cross_confirm_bars + 1, len(dc_mid))):
            if pd.isna(dc_mid.iloc[-(i+1)]) or pd.isna(ma_200.iloc[-(i+1)]) or pd.isna(dc_mid.iloc[-i]) or pd.isna(ma_200.iloc[-i]):
                continue
                
            # 金叉检测：DC28中轨从下方穿过MA200向上
            if (dc_mid.iloc[-(i+1)] <= ma_200.iloc[-(i+1)] and dc_mid.iloc[-i] > ma_200.iloc[-i]):
                signal_info["golden_cross"] = True
                signal_info["cross_bars_ago"] = i
                break
                
            # 死叉检测：DC28中轨从上方穿过MA200向下  
            elif (dc_mid.iloc[-(i+1)] >= ma_200.iloc[-(i+1)] and dc_mid.iloc[-i] < ma_200.iloc[-i]):
                signal_info["death_cross"] = True
                signal_info["cross_bars_ago"] = i
                break
        
        # 验证价格位置
        cross_point = (signal_info["dc_mid_current"] + signal_info["ma200_current"]) / 2
...
def scan_symbols(base: str, symbols: List[str], granularity: str, signal_types: List[str], cross_confirm_bars: int, min_volume: float = 0, price_position_strict: bool = True) -> Tuple[List[dict], dict]:
    """扫描交易对的DC-MA交叉信号"""
    ...
    with st.spinner("🧮 正在计算交叉信号..."):
        ...
        for symbol in symbols:
            try:
                ...
                signal_info, candle_count = calculate_dc_ma_signals(df, cross_confirm_bars, price_position_strict)
                ...
                if has_golden_cross and "金叉信号" in signal_types:
                    ...
                    results.append({
                        ...
                        "ma200": signal_info["ma200_current"]
                    })
...
def main():
    ...
    # 创建侧边栏并获取参数
    timeframe, signal_types, cross_confirm_bars, show_charts, min_volume, price_position_strict = create_sidebar()
    ...
    with col2:
        # 扫描按钮
        if st.button("🚀 开始扫描", key="scan_button", help="点击开始扫描DC28×MA200交叉信号"):
            scan_pressed = True
        else:
            scan_pressed = False
    ...
    with col1:
        if not scan_pressed:
            ...
            **DC28×MA200交叉扫描器**专门用于检测唐奇安通道与移动平均线的交叉信号：
            
            #### 📊 核心指标：
            - 🔷 **DC28通道**: 28日唐奇安通道，中轨=(最高价+最低价)/2
            - 📈 **MA200均线**: 200日移动平均线，长期趋势基准
            - ⚡ **交叉检测**: 实时监控两线交叉状态
            
            #### 🎯 交易信号：
            - 🟢 **金叉信号**: DC28中轨上穿MA200 + 价格在交叉点上方 = 看涨
            - 🔴 **死叉信号**: DC28中轨下穿MA200 + 价格在交叉点下方 = 看跌
    ...
    if scan_pressed:
        ...
        try:
            ...
            # 执行扫描
            results, scan_stats = scan_symbols(base, symbols, timeframe, signal_types, cross_confirm_bars, min_volume, price_position_strict)
            
            # 显示扫描统计
            st.success(f"✅ 扫描完成! 耗时 {scan_stats['scan_time']:.1f} 秒")
            
            if scan_stats['insufficient_data'] > 0:
                st.info(f"ℹ️ 有 {scan_stats['insufficient_data']} 个币种数据不足(需要{Config.MIN_CANDLES_RELIABLE}+根K线)，已跳过")
            ...
            if "金叉信号" in signal_types:
                st.markdown(f"### 🟢 金叉信号 (DC28中轨上穿MA200 - {timeframe})")
                if golden_crosses:
                    ...
                    st.download_button(
                        label="📥 下载金叉信号 CSV",
                        data=csv_data,
                        file_name=f"golden_cross_dc28_ma200_{timeframe}_{current_time.replace(' ', '_').replace(':', '-')}.csv",
                        mime="text/csv",
                        key="download_golden"
                    )
                else:
            ...
            if "死叉信号" in signal_types:
                st.markdown(f"### 🔴 死叉信号 (DC28中轨下穿MA200 - {timeframe})")
                if death_crosses:
                    ...
                    st.download_button(
                        label="📥 下载死叉信号 CSV", 
                        data=csv_data,
                        file_name=f"death_cross_dc28_ma200_{timeframe}_{current_time.replace(' ', '_').replace(':', '-')}.csv",
                        mime="text/csv",
                        key="download_death"
                    )
                else:
